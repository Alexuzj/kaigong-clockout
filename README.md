...
第一次在一个有效项目里开工时，Skill 会先完成三行开工结果，再问一次：

> 我找到了这个项目。要用“我的网站”这个名字记住吗？

确认一次后，以后可以直接说项目名，不需要记住电脑路径：
...
私人项目目录只保存在本机。普通列表只显示项目名，不显示绝对路径；项目改名、归档或移除只修改目录记录，不会改动真实文件。
...
Skill 会先区分本次改动和开工前已有改动，更新必要的交接，再检查最终文件。如果检查失败，它会停下来说明问题，不会提交、推送或假装完成。本轮没有改动或新决定时，也不会为了“收工”而改写记录。
...
- 不会把“管理项目目录”偷换成移动、重命名或删除真实文件

<details>
<summary>高级设置：手动配置私人项目目录</summary>

目录位于 `$CODEX_HOME/kaigong-clockout/projects.json`；如果没有设置 `CODEX_HOME`，macOS/Linux 默认是 `~/.codex/kaigong-clockout/projects.json`，Windows 默认在 `%USERPROFILE%\.codex\kaigong-clockout\projects.json`。
...
这个 Skill 经过多轮第一性原理梳理、陌生用户测试和对抗性审查，重点检查了错误项目、陈旧记录、原有改动、失败验证、项目搬家、隐私和越权交付。
